...
def plot_input_LLM_and_SF(results):
    ...
    axes[0].plot(time, results[f'agent {str(len(results)-1)}']['acc pred SF'])
    axes[0].plot(time, results[f'agent {str(len(results) - 1)}']['acc pred LLM'])
...
def plot_vehicles(results, fig, ax, env, time):
    ...
    def update(frame):
        ...
        for id_agent in vehicles:
            x = results[f'agent {id_agent}']['x coord'][frame] - \
                env['Vehicle Specification'][results[f'agent {id_agent}']['type']]['length'] / 2
            y = results[f'agent {id_agent}']['y coord'][frame] - \
