...
counter = {'crash': 0,
           ...
           'too_near': 0,
           'recall due to MPC LLM': 0}

type_simulation = "llm_conditioned_mpc"
...
while run_simulation:
    ...
    if SimulationParam['With LLM car']:
        print('Action: ', LLM.TP['action chosen'][0])
        print('Reason: ', LLM.TP['reasons'][0])
        ...
        #        next_task = True
            ...
            if 'go to the entry' in LLM.TP['action chosen'][0]:
                LLM.OD = {
                    ...
                    "inequality_constraints": ["X[3] - " + str(ego_vehicle.entry['max vel'])]}
            elif 'go to the exit' in LLM.TP['action chosen'][0]:
                LLM.OD = {
                    "objective": "ca.norm_2(X - self.exit['state'])**2",
                    "equality_constraints": [],
                    "inequality_constraints": ["X[3] - " + str(ego_vehicle.exit['max vel'])]}
            elif 'go to the final_target' in LLM.TP['action chosen'][0]:
                LLM.OD = {
                    "objective": "ca.norm_2(X - self.final_target['state'])**2",
                    "equality_constraints": [],
                    "inequality_constraints": ["X[3] - " + str(ego_vehicle.final_target['max vel'])]}
            else:
    ...
    if SimulationParam['With LLM car']:
        ...
        if 'entry' in LLM.TP['action chosen'][0]:
            ...
            if np.linalg.norm(ego_vehicle.position - ego_vehicle.entry['position']) <= 1:
                next_task = True
        elif 'exit' in LLM.TP['action chosen'][0]:
            ...
            if np.linalg.norm(ego_vehicle.position - ego_vehicle.exit['position']) <= 1:
                next_task = True
        elif 'final_target' in LLM.TP['action chosen'][0]:
            ego_vehicle.inside_cross = False
        ...
        if run_simulation:
            ...
            #    counter['LLM calls'] += 1
                ...
                if replan_flag == True:
                    ...
                    counter['elapsed time for LLM'].append(end - start)
                    counter['LLM calls'] += 1
                    counter['recall due to MPC LLM'] += 1
                    ego_vehicle.success_solver_MPC_LLM = True
...
print('How many times LLM is called due to MPC LLM: ', counter['recall due to MPC LLM'])
