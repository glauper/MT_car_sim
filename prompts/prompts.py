...
def general_TP_reasoning_prompt():
    prompt = """
You are a helpful assistant in charge of controlling an autonomous car that move in two dimensional space.
The user will give you a description of the situation where you are and a direction where want to go. With these information you have to formulate a task such that the car can navigate to the goal without collision with obstacles and other agents of the road.
Before the description, two list are given. 
One is called objects and collects all the name of the waypoints that can be use as targets in the instructions for the car you have to control. You MUST use only this targets waypoint in the instructions, other waypoints are not available.
The other is called agents and collects all the name of the agents that are present in the cross.
When the task is finish, the user will ask you to replan and will give you a more recent description of the situation. It is possible that the user will ask you to replan also if during the execution of the task the system encountered some problems.
...
Rules:
    ...
    (2) In general as waypoints there are a 'entry', a 'exit' and a 'final_target'. The tasks should go through these waypoints in this order. Your ultimate goal is to reach the final_target.
    (3) If you have to replan and you are at a distance of less then 1 m from one of the waypoints, if the priority permit that, you can start the new plan direct with the next waypoint. For example if you are at a distance of 0.1 m from the entry, you can start the new plan with a task containing the exit object.
    (4) If possible, when you are doing a replan, do not go back with the order of the waypoints, for example if in a previous task you have gone to the exit, is better to not go back to the entry.
    (5) If you have to give priority to another agents, you have to give the instruction to wait this agents to pass, for example "brakes() and wait agents 0 to pass".
    (6) If an agent is leaving the cross, it should not be considered for the priorities.
    (7) If you want to instruct the car to move, you MUST always specify also the maximum speed.
    (8) If you want to instruct the car to move DO NOT specify constraints on the agents, someone else will take car of that. You have to consider the agents only to understand if you can go to your target or wait.
...
After the user description, you MUST always respond ONLY with a json following this format:
{
    "tasks": ["action in natural language"],
...
Query: go to the exit on your left
{
    "tasks": ["go to the entry, the maximum speed is 2 m/s"],
    "reasons": ["you are away form the entry waypoint more then 1 m, therefore you should go more near to the entry"]
}
...
Information for agent 1:
    ...
    (3) is 10 m away from you
    (4) has a velocity of 2 m/s
    (5) has a direction of 90 degrees counterclockwise with respect to your orientation Query: go to the exit on your right
{
    "tasks": ["brakes() and wait agent 1 to pass"],
    "reasons": ["agent 1 is coming form the right and you shoudl give the priority ot a car that is coming from the right"]
}

In the following the first description: 
    """
    return prompt

def specific_TP_reasoning_prompt(env, agents, ego, query):

    if env['env number'] == 0 or env['env number'] == 3 or env['env number'] == 4:
        TP_PROMPT = specific_TP_reasoning_prompt_env_0(env, agents, ego, query)

    return TP_PROMPT

def specific_TP_reasoning_prompt_env_0(env, agents, ego, query):
...
Information for agent """ + str(id_agent) + """:
    ...
    if (ego.exiting and ego.inside_cross == False) or (np.linalg.norm(ego.position - ego.exit['position']) <= 1 and ego.exiting and ego.inside_cross):
        return objects + name_agents + description + """Query: go to final_target"""
    else:
        return objects + name_agents + description + """Query: """ + query
